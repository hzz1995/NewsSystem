...
$(function(){
    ...
    tabClose();
    tabCloseEven();
    $('#tabs').tabs({//关闭当前，选中上一次选中的tabs
    	onSelect: function(title,index) {
    		lastTabs = $.grep(lastTabs, function(n, i) { return n != title; }); 
    		//重新压入，保证 最新的在最上面 
    		lastTabs.push(title);     		
    		currentTabTitle = title; //更新当前 
    	},
    	onClose: function(title,index){
    		lastTabs = $.grep(lastTabs, function(n, i) { return n != title; });     		 
    		$('#tabs').tabs('select', lastTabs[lastTabs.length - 1]);//重新选择
    	}
    })
});

function addTab(id,subtitle,iframeurl){//只允许显示一个tab
    if(!$('#tabs').tabs('exists',subtitle)){
        ...
        $('#tabs').tabs('getSelected').css('height','auto');//
...
{
    var s = '<iframe id="mainFramee'+id+'" name="mainFrame" scrolling="auto" height="100%" width="100%" style="width:100%;height:100%;" frameborder="0"  src="'+url+'" ></iframe>';
    return s;
}
